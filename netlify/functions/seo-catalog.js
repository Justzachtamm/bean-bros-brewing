...
exports.handler=async event=>{
 const headers={'Content-Type':'text/html; charset=utf-8','Cache-Control':'public, max-age=0, must-revalidate','X-Content-Type-Options':'nosniff','Strict-Transport-Security':'max-age=63072000; includeSubDomains; preload'};
 ...
 try{
  ...
  if(parts.length!==2||parts[1]!==seo.slug(p)||(pathname.startsWith('/products/')&&pathname!==new URL(seo.url(p)).pathname))return {statusCode:301,headers:{...headers,Location:seo.url(p)},body:''};
