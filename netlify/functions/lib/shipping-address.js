...
function shippingAddress(value, {partial=false} = {}) {
  ...
  if ((!partial&&(!result.name || !result.address)) || !result.city || !STATES.has(result.state) || !/^\d{5}(-\d{4})?$/.test(result.zip) || result.country !== 'US') throw Error('Enter a complete US delivery address, including state and ZIP code.');
