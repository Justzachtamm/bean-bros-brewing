...
function renderBag(){const locked=checkingOut&&!preparingPayment;$('delivery-dialog').querySelector('[aria-label="Close checkout"]').disabled=false;$('email-subscription-news').disabled=locked;$('email-promotions').disabled=locked;renderRecommendations();$('promo-code').disabled=paymentSubmitting;$('apply-promo').disabled=paymentSubmitting||promoApplying;$('payment-email').disabled=locked;$('billing-same').disabled=locked;$('edit-delivery').disabled=locked;$('pay-order').disabled=locked;$('back-to-bag').disabled=false;$('shipping-form').querySelectorAll('input,select,button').forEach(el=>el.disabled=locked||(el.id==='get-shipping'&&shippingBusy));if(shippingQuote&&shippingQuote.key!==quoteKey())clearShipping();const count=bag.reduce((a,i)=>a+i.quantity,0),total=bag.reduce((a,i)=>a+Math.round(subscriptionItemPrice(i.product,i.isSubscription)*100)*i.quantity,0)/100;$('bag-count').textContent=count;$('cart-open').setAttribute('aria-label',`Open bag, ${count} items`);$('drawer-count').textContent=' ('+count+')';$('shipping-progress').hidden=threshold===null;$('shipping-progress').max=threshold||1;$('shipping-progress').value=Math.min(total,threshold||0);$('shipping-message').textContent=threshold===null?'Shipping confirmed at checkout.':total>=threshold?'Your order qualifies for free standard shipping.':money(threshold-total)+' away from free standard shipping.';$('cart-items').innerHTML=bag.length?bag.map((i,n)=>`<article class="cart-item"><img src="${imageURL(i.product)}" alt=""><div><h3>${escapeHTML(i.product.name.trim())}</h3><p>${escapeHTML(i.grindLabel)}</p><p>${i.isSubscription?escapeHTML(i.frequencyLabel)+((i.product.category||'coffee')==='coffee'?' · 10% off':''):'One-time purchase'}</p><label>Qty <input class="cart-quantity" aria-label="Quantity for ${escapeHTML(i.product.name)}" type="number" min="1" max="${Math.min(50,i.product.stock)}" value="${i.quantity}" data-quantity="${n}" ${checkingOut?'disabled':''}></label><button class="remove-item" aria-label="Remove ${escapeHTML(i.product.name)} from bag" data-remove="${n}" ${checkingOut?'disabled':''}>Remove</button></div><strong>${money(Math.round(subscriptionItemPrice(i.product,i.isSubscription)*100)*i.quantity/100)}</strong></article>`).join(''):'<p class="empty-bag">Your next favorite coffee belongs here. Choose a bag to get started.</p>';
