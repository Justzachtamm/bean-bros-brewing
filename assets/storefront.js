...
function productURL(p){return `/products/${p.id}/${String(p.name).normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}/`}
function imageURL(p){return window.BeanBrosImage(originalImageURL(p))}
...
$('delivery-dialog').addEventListener('close',()=>{checkoutStartRevision++;clearTimeout(checkoutTimer);autoCheckoutKey='';window.BeanBrosExpress.destroy();resetPayment();window.BeanBrosPayment.destroy();checkingOut=false;paymentSubmitting=false;renderBag()});
...
$('edit-delivery').onclick=()=>{resetPayment();renderBag();$('payment-email').focus()};
...
function pricedBag(){return window.BeanBrosMugOffer.apply(bag.map((i,index)=>({index,productId:i.product.id,quantity:i.quantity,price:subscriptionItemPrice(i.product,i.isSubscription),isSubscription:i.isSubscription})));}
function bagLineTotal(index){return pricedBag().filter(i=>i.index===index).reduce((sum,i)=>sum+Math.round(i.price*100)*i.quantity,0)/100;}
function renderBag(){const locked=checkingOut&&!preparingPayment;$('delivery-dialog').querySelector('[aria-label="Close checkout"]').disabled=false;$('email-subscription-news').disabled=locked;$('email-promotions').disabled=locked;renderRecommendations();$('promo-code').disabled=paymentSubmitting;$('apply-promo').disabled=paymentSubmitting||promoApplying;$('payment-email').disabled=locked;$('billing-same').disabled=locked;$('edit-delivery').disabled=locked;$('pay-order').disabled=locked;$('back-to-bag').disabled=false;$('shipping-form').querySelectorAll('input,select,button').forEach(el=>el.disabled=locked||(el.id==='get-shipping'&&shippingBusy));if(shippingQuote&&shippingQuote.key!==quoteKey())clearShipping();const count=bag.reduce((a,i)=>a+i.quantity,0),total=pricedBag().reduce((a,i)=>a+Math.round(i.price*100)*i.quantity,0)/100;$('bag-count').textContent=count;$('cart-open').setAttribute('aria-label',`Open bag, ${count} items`);$('drawer-count').textContent=' ('+count+')';$('shipping-progress').hidden=threshold===null;$('shipping-progress').max=threshold||1;$('shipping-progress').value=Math.min(total,threshold||0);$('shipping-message').textContent=threshold===null?'Shipping confirmed at checkout.':total>=threshold?'Your order qualifies for free standard shipping.':money(threshold-total)+' away from free standard shipping.';$('cart-items').innerHTML=bag.length?bag.map((i,n)=>`<article class="cart-item"><img src="${imageURL(i.product)}" alt=""><div><h3>${escapeHTML(i.product.name.trim())}</h3><p>${escapeHTML(i.grindLabel)}</p>${i.product.id===window.BeanBrosMugOffer.productId?'<p>Buy 2, save $10 · applied automatically</p>':''}<p>${i.isSubscription?escapeHTML(i.frequencyLabel)+((i.product.category||'coffee')==='coffee'?' · 10% off':''):'One-time purchase'}</p><label>Qty <input class="cart-quantity" aria-label="Quantity for ${escapeHTML(i.product.name)}" type="number" min="1" max="${Math.min(50,i.product.stock)}" value="${i.quantity}" data-quantity="${n}" ${checkingOut?'disabled':''}></label><button class="remove-item" aria-label="Remove ${escapeHTML(i.product.name)} from bag" data-remove="${n}" ${checkingOut?'disabled':''}>Remove</button></div><strong>${money(bagLineTotal(n))}</strong></article>`).join(''):'<p class="empty-bag">Your next favorite coffee belongs here. Choose a bag to get started.</p>';
...
$('checkout-live').onclick=async()=>{
 ...
 try{
  ...
  if(await window.BeanBrosPayment.prepare(data,deliveryAddress(),busy=>{checkingOut=busy;paymentSubmitting=busy;renderBag()})){
...
$('begin-checkout').onclick=async()=>{
 ...
 try{const [initial]=await Promise.all([api('create-checkout-session',{action:'wallet-start',items:checkoutItems()}),window.BeanBrosPayment.preload()]);if(revision!==checkoutStartRevision||!$('delivery-dialog').open)return;const config={publishableKey:initial.publishableKey};
   try{await window.BeanBrosExpress.start(config.publishableKey,{initial,
   ...
   });}catch(e){if(revision!==checkoutStartRevision||!$('delivery-dialog').open)return;$('wallet-note').textContent='Express checkout could not load. Retry or enter your card details below.';$('wallet-retry').hidden=false;$('express-error').textContent=e.message;showManualCheckout()}
...
$('navigation').addEventListener('keydown',e=>{if(e.key==='Escape'){$('navigation').classList.remove('open');$('menu-open').setAttribute('aria-expanded','false');$('menu-open').focus()}});
...
function renderHerbs(){const list=products.filter(p=>isHerb(p)&&p.active);$('herb-products').innerHTML=list.length?list.map(p=>`<article class="product-card"><button type="button" class="product-image product-photo-button" data-photo-product="${p.id}" aria-label="View photos and details for ${escapeHTML(p.name.trim())}"><img src="${imageURL(p)}" alt="${escapeHTML(p.name)} package" loading="lazy" decoding="async" width="600" height="800"></button><div class="product-topline"><span>Herbs & tea</span><span>${escapeHTML(p.weight)}</span></div><h3><a href="${productURL(p)}">${escapeHTML(p.name)}</a></h3><p class="tasting-notes">${escapeHTML(p.tastingNotes)}</p><div class="product-bottom"><span class="price">${money(p.price)}</span><button class="choose-button" aria-label="Choose options for ${escapeHTML(p.name.trim())}" data-herb="${p.id}" ${p.stock<1?'disabled':''}>${p.stock<1?'Sold out':'Choose options +'}</button></div></article>`).join(''):'<p>Herbs and teas will be available shortly.</p>';document.querySelectorAll('[data-herb]').forEach(b=>b.onclick=()=>showProduct(Number(b.dataset.herb)))}
...
function showSubscriptionBuilder(){
 ...
 $('builder-back').onclick=()=>{step=1;$('subscription-coffee-step').hidden=false;$('subscription-mushrooms').hidden=true;$('builder-back').hidden=true;$('builder-next').textContent='Next →';$('subscription-coffee-step').querySelector('h2').focus()};
...
$('apply-promo').onclick=async()=>{
 ...
 const code=$('promo-code').value.trim(),attempt=++promoAttempt;
 if(!code){$('promo-status').textContent='Enter a promo code.';return}
 promoApplying=true;$('apply-promo').disabled=true;$('promo-status').textContent='Checking promo code…';
 try{
  const result=await api('create-checkout-session',{action:'promo-check',items:checkoutItems(),promoCode:code});
  if(attempt!==promoAttempt)return;
  $('promo-status').textContent=result.code+' applied — '+money(result.discount/100)+' off your items. '+(checkingOut?'Close and reopen your wallet to use this code.':'Your final total will include shipping and tax.');
  autoEditVersion++;autoCheckoutKey='';resetPayment();scheduleCheckout();
 }catch(e){if(attempt===promoAttempt)$('promo-status').textContent=e.message}
 finally{promoApplying=false;renderBag()}
};
$('promo-code').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();$('apply-promo').onclick()}});
...
// Escape uses the same close cleanup as Close checkout and Back to bag.
