...
test('wallet startup includes the public key and skips shipping configuration',async()=>{const x=checkoutHarness({STRIPE_SECRET_KEY:'sk_test_example',STRIPE_PUBLISHABLE_KEY:'pk_test_example',STRIPE_TAX_ENABLED:'true'},'coffee',true,true);const r=await x.h(event({action:'wallet-start',items:cart.items}));assert.equal(r.statusCode,200);const d=JSON.parse(r.body);assert.equal(d.publishableKey,'pk_test_example');assert.equal(d.amount,2000);assert.equal(x.config(),undefined)});
