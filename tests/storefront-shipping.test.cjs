...
for(const subscription of [true,false])test(`checkout retains address and shipping while controls are disabled (${subscription?'subscription':'one-time'})`,async()=>{
 ...
 const handler=source.slice(source.indexOf("$('checkout-live').onclick="),source.indexOf("$('cart-open').onclick="));
 vm.runInContext(handler,context);
 await nodes['checkout-live'].onclick();
 assert.equal(request.shipTo.zip,'10118');assert.equal(request.shipTo.residential,false);assert.equal(request.shippingAmount,777);assert.equal(request.shippingService,'03');assert.equal(redirect,'https://checkout.stripe.com/test');assert.equal(context.checkingOut,false);
});
