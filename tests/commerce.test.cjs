...
const shipTo={name:'Customer',address:'123 Test St',address2:'Apt 2',city:'Newark',state:'NJ',zip:'07102',country:'US'};
...
test('checkout uses catalog price, captures receipt proof and preserves selected shipping service',async()=>{
 let config;
 const stripe={customers:{create:async()=>({id:'cus_test'})},checkout:{sessions:{create:async data=>{config=data;return{id:'cs_test_valid',url:'https://checkout.stripe.com/test'}}}}};
 ...
 const body={shipTo,shippingService:'02',shippingAmount:1000,items:[{name:'Coffee',price:1,quantity:2,grindLabel:'Whole Bean'}],successUrl:'https://beanbrosbrewingco.com/#/checkout-success',cancelUrl:'https://beanbrosbrewingco.com/#/cart'};
...
test('shipping label uses expedited service, apartment and private storage',async()=>{
 ...
 const order={id:'BB-LABEL',sessionId:'cs_test_label',shippingService:'02',shippingPackage:{weightLbs:1.5,packagingCode:'02',dimensions:{length:'14',width:'6',height:'3'}},items:[{quantity:1}],shippingAddress:{name:'Test',address:'123 Test St',address2:'Apt 2',city:'Test',state:'NJ',zip:'00000',country:'US'}};
...
test('herb checkout uses the catalog package, price and stock and rejects subscriptions',async()=>{let config;const handler=loader({stripe:()=>({customers:{create:async()=>({id:'cus_test'})},checkout:{sessions:{create:async c=>{config=c;return{id:'cs_test_herb',url:'https://checkout.stripe.com/test'}}}}}),'./lib/products':{getProductByName:async()=>({id:900001,name:'Sage Leaf',category:'tea',weight:'1 oz',price:10,stock:2,active:true})},'./lib/accounts':{requireSession:async()=>verified},'./lib/shipping-config':{getShippingConfig:async()=>({freeShipThreshold:50})},'./lib/ups':{validateAddress:async()=>({verified:true})},'./lib/shipping-rates':{getPackageDetails:()=>({}),getShippingOptions:async()=>[{serviceCode:'03',displayName:'UPS Ground',amountCents:800,minDays:1,maxDays:5}]}})('netlify/functions/create-checkout-session.js').handler;const body={shipTo,shippingService:'03',shippingAmount:800,items:[{name:'Sage Leaf',quantity:1,grind:'fake',price:1}],successUrl:'https://beanbrosbrewingco.com/#/checkout-success',cancelUrl:'https://beanbrosbrewingco.com/#/cart'};assert.equal((await handler(event(body))).statusCode,200);assert.equal(config.line_items[0].price_data.unit_amount,1000);assert.equal(config.line_items[0].price_data.product_data.description,'1 oz');body.items[0].isSubscription=true;assert.equal((await handler(event(body))).statusCode,400)});
test('subscription list stays within Stripe expansion limits and resolves product IDs once',async()=>{
 const products=[];const items=[coffee(),coffee('si_second'),shipping()];for(const item of items)item.price.product=item.price.product.id;
 const stripe={subscriptions:{list:async args=>{for(const expansion of args.expand)assert.ok(expansion.split('.').length<=4,'Stripe rejects deep expansions');return{data:[{id:'sub',status:'active',items:{data:items},current_period_end:1900000000}]}}},products:{retrieve:async id=>{products.push(id);return id==='prod_ship'?shipping().price.product:coffee().price.product}}};
 const rows=await loader()('netlify/functions/lib/subscriptions.js').listCustomerSubscriptionItems(stripe,'cus');assert.equal(rows.length,2);assert.equal(rows[0].productName,'Coffee');assert.deepEqual(products,['prod_coffee','prod_ship']);
});
test('subscription item billing dates work when Stripe omits the legacy subscription date',async()=>{
 const item=coffee();item.current_period_end=1900000000;const stripe={subscriptions:{list:async()=>({data:[{id:'sub',status:'active',items:{data:[item]}}]})}};
 const rows=await loader()('netlify/functions/lib/subscriptions.js').listCustomerSubscriptionItems(stripe,'cus');assert.equal(rows[0].nextDelivery,new Date(1900000000000).toISOString());
});
